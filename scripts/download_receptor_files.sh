...
for excape_gene in "${!excape2dude[@]}"; do
    ...
    wget http://dude.docking.org/targets/"$dude_gene"/receptor.pdb
    mv receptor.pdb "$excape_gene"_receptor_original.pdb
    # Format the PDB
    obabel -ipdb "$excape_gene"_receptor_original.pdb -opdb -O "$excape_gene"_receptor_obabel.pdb
    cat "$excape_gene"_receptor_obabel.pdb | grep -v "CONECT" | grep -v "MASTER" > "$excape_gene"_receptor_grep.pdb
    cp "$excape_gene"_receptor_grep.pdb "$excape_gene"_receptor.pdb
done
